...
rmt_stepper_curve_encoder_t step_encoder;
static const char *TAG = "stepper_motor_encoder";


float get_sine_sample(float frequency, int tot_n, int n)
{
    // Calculate the time for the given sample
    float t = (float)n / (float)tot_n;
    
    // Calculate the phase angle for the given time and frequency
    float phase = 2.0 * M_PI * frequency * t;
    
    // Calculate the sine value for the given phase angle
...
esp_err_t rmt_new_stepper_motor_custom_encoder(const stepper_motor_custom_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder)
{
    esp_err_t ret = ESP_OK;
    float smooth_freq;
    uint32_t symbol_duration;
    ESP_GOTO_ON_FALSE(config && ret_encoder, ESP_ERR_INVALID_ARG, err, TAG, "invalid arguments");
    ESP_GOTO_ON_FALSE(config->sample_points, ESP_ERR_INVALID_ARG, err, TAG, "sample points number can't be zero");
    ESP_GOTO_ON_FALSE(config->sample_points <= MAX_SAMPLE_POINTS, ESP_ERR_INVALID_ARG, err, TAG, "sample points exceed max limit");

    memset(&step_encoder, 0, sizeof(step_encoder));
    ...
    for (int i = 0; i < config->sample_points; i++) {
        smooth_freq = (uint32_t)config->curve[i];
        uint32_t ui = (uint32_t)i;
        symbol_duration = config->resolution / smooth_freq / 2;
        step_encoder.curve_table[ui].level0 = 0;
        step_encoder.curve_table[ui].duration0 = symbol_duration;
        step_encoder.curve_table[ui].level1 = 1;
        step_encoder.curve_table[ui].duration1 = symbol_duration;
    }
...
static size_t rmt_encode_stepper_motor_uniform(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    uint32_t symbol_duration = ((motor_encoder->resolution / target_freq_hz) - 40 ) / 2;
